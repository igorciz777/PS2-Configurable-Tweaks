...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-slate-850 via-blue-850 to-slate-800">
      {/* ── Top Bar ── */}
      <header className="flex flex-wrap items-center justify-between px-4 sm:px-8 py-3 sm:py-5 border-b border-slate-800/60 backdrop-blur-sm select-none bg-slate-950/80">
        <div className="flex items-center gap-3 sm:gap-5">
          <h1 className="text-base sm:text-xl font-semibold" style={{ color: '#e0e4f0', letterSpacing: '0.02em' }}>PS2 Configurable Tweaks</h1>
          <div className="w-px h-6 hidden sm:block" style={{ background: 'rgba(80,90,160,0.2)' }} />
        </div>
        <div className="flex items-center gap-2 sm:gap-3 mt-2 sm:mt-0">
          <button
            onClick={() => { setPatcherPnach(''); setPatcherOpen(true); }}
            className="font-sans text-xs font-semibold uppercase tracking-wider px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg transition-all duration-150"
            style={{
              color: '#40c080',
      ...
      {/* ── Main Content ── */}
      <main className="flex-1 flex flex-col min-[1366px]:flex-row gap-6 sm:gap-8 p-4 sm:p-8" style={{ maxWidth: '1440px', margin: '0 auto', width: '100%' }}>
        {/* ── Sidebar ── */}
        ...
        <section className="flex-1 flex flex-col gap-6" style={{ minWidth: 0 }}>
          {deadzoneFields.length > 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-1 h-auto sm:h-64">
